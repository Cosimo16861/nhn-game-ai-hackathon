 ...
 *   데이터   src/cutscenes/data/beats-l3-l4.js
 *   렌더러   src/cutscenes/renderers/l3-l4.js
 *   재생기   src/cutscenes/cutscene-player.js
 *
 * ?bundle= 로 묶음을 고르고, ?main=q3a-complete 로 조건부 맺음을 확인한다.
...
(function (global) {
  ...
  const bundleId = BUNDLE_BY_KEY[params.get("bundle")] || "B_AFTER_Q3A";
  ...
  const clearedForReview = params.get("main") === "q3a-complete" ? ["Q3A_SEAL"] : [];
  ...
  const player = global.CutscenePlayer.create({
    ...
    onSceneComplete(sceneId, info) {
      const scene = global.CutsceneBeatsL3L4[sceneId];
