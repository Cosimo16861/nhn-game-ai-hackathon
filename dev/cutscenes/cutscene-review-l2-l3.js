 ...
 *   데이터   src/cutscenes/data/beats-l2-l3.js
 *   렌더러   src/cutscenes/renderers/l2-l3.js
 *   재생기   src/cutscenes/cutscene-player.js
 *
 * ?bundle= 로 묶음을 고르고, ?q2a=complete 로 조건부 맺음을 확인한다.
...
(function (global) {
  ...
  const bundleId = BUNDLE_BY_KEY[params.get("bundle")] || "B_AFTER_Q2A";
  ...
  const clearedForReview = params.get("q2a") === "complete" ? ["Q2A_TRUE_FACE"] : [];
  ...
  const player = global.CutscenePlayer.create({
    ...
    onSceneComplete(sceneId, info) {
      const scene = global.CutsceneBeatsL2L3[sceneId];
