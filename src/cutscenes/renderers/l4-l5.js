/**
 * L4-L5 컷신 렌더러 — B_AFTER_Q4A · B_AFTER_Q4B · B_AFTER_Q4C.
 *
 * dev/cutscenes/cutscene-review-l4-l5.js 의 시각 코드를 그대로 옮겼다.
...
(function (global) {
    ...
    return {
      ...
      speakerStyle(speaker) {
        ...
        const accent = speaker === "플레이어" ? PAL.teal
          : speaker === "리드" ? PAL.blue
            : speaker === "뱅크스" ? "#8FA7A3" : PAL.rust;
        const text = speaker === "플레이어" ? "#9BC0B6"
          : speaker === "리드" ? "#9FC3C7"
            : speaker === "뱅크스" ? "#B5C7BF" : "#D28B73";
        return { accent, text };
      },
  ...
  const RENDERER_ID = "l4-l5";
  global.CutsceneRenderer_l4_l5 = Object.freeze({ id: RENDERER_ID, ASSETS, create });
