...
640×384 캔버스 한 장으로 다시 만들었다. 해상도는 `게임배경이미지.png`의 종횡비가
640:384와 일치하는 것을 근거로 잡았다.
...
(256²는 `Q1A`·`Q2A`·`Q5A` 셋뿐, 나머지는 16²~128². `GAME_DESIGN` 8.6).
...
| 질문 | 선택 | 함의 |
| --- | --- | --- |
| 256²에서 무엇을 그리는가 | **인물 전체를 채색** | 선화만 제공하고 전체를 채운다. 단 256²는 세 노드뿐 |
| 정답 그림은 무엇으로 만드는가 | 기존 자산 + 절차 생성 | 전제가 9장에서 무너졌다. 재확인 필요 |
