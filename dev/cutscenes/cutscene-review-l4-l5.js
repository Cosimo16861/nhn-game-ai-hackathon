 ...
 *   데이터   src/cutscenes/data/beats-l4-l5.js
 *   렌더러   src/cutscenes/renderers/l4-l5.js
 *   재생기   src/cutscenes/cutscene-player.js
 *
 * ?bundle= 로 묶음을 고르고, ?q5a=1 로 조건부 맺음을 확인한다.
...
(function (global) {
  ...
  const bundleId = BUNDLE_BY_KEY[params.get("bundle")] || "B_AFTER_Q4A";
  ...
  const clearedForReview = params.get("q5a") === "1" ? ["Q5A_DOCK"] : [];
  ...
  const player = global.CutscenePlayer.create({
    ...
    onSceneComplete(sceneId, info) {
      const scene = global.CutsceneBeatsL4L5[sceneId];
